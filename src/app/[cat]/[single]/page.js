                    ...
                    <nav aria-label="breadcrumb">
                        <ol className="breadcrumb">
                            <li className="breadcrumb-item"><a href="/"><i className="fa fa-home" aria-hidden="true"></i>
                                    الرئيسية</a></li>
                            <li className="breadcrumb-item"><a href={content.data[0]?.attributes.mainCategory.data.attributes.path}>{content.data[0]?.attributes.mainCategory.data.attributes.title}</a></li>
                            <li className="breadcrumb-item active" aria-current="page">{content.data[0]?.attributes.title}
                            </li>
                        </ol>
                    </nav>
                </div>
                    ...
                    <div className="post-details-content">
                        ...
                        <div className="blog-content">
                            ...
                            <div className="post-content mt-0">
                                ...
                                <a href="single-post.html" className="post-title mb-2">{content.data[0]?.attributes.title}</a>

                                <div className="d-flex justify-content-between mb-30">
                                    <div className="post-meta d-flex align-items-center">
                                        <a href="#" className="post-author">By Jane</a>
                                        <i className="fa fa-circle" aria-hidden="true"></i>
                                        <a href="#" className="post-date">  { new Date(content.data[0]?.attributes.publishedAt).toISOString().split("T")[0]}</a>
                                    </div>
                                    <div className="post-meta d-flex">
...
{content.data[0]?.attributes.content}
                            ...
                            <div className="related-post-area mt-5">
                                ...
                                <div className="row">
                                {news?.map(e=>(<div key={e.id} className="col-12 col-md-6">
                                        <div className="single-post-area mb-50 bg-white">
                                            ...
                                            <div className="post-content p-3">
                                                <a href={"/"+e.attributes.mainCategory.data.attributes.path} className="post-cata cata-sm cata-success">{e.attributes.mainCategory.data.attributes.title}</a>
                                                <a href={`/${e.attributes.mainCategory.data.attributes.path}/${e.attributes.title}`} className="post-title">{e.attributes.title}</a>
...
export async function generateStaticParams() {
    ...
    const news = await fetch(`${BASE_URL}/api/news?populate=*&pagination[pageSize]=100`,{ headers ,next: { revalidate: 10 }}).then((res) => res.json())
    let data=[];
    for(let i=1; i<=news.meta.pagination.pageCount;i++)
    {
       let res= await fetch(`${BASE_URL}/api/news?populate=*&pagination[pageSize]=100&pagination[page]=${i}`,{ headers ,next: { revalidate: 10 }}).then((res) => res.json())
    data.push(...res.data)
    ...
    return data?.map((n) => ({
       cat: n.attributes.mainCategory.data.attributes.path,single:encodeURIComponent( n.attributes.slug)
  ...
  async function getContent(single) {
    const res = await fetch(`${BASE_URL}/api/news?populate=*&filters[slug][$eq]=${single}`,{ headers})
